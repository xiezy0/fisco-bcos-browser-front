  ...
  <div class="home">
    <div  style="padding:20px">
      <div class="home-head">
        ...
        <div
          class="home-head-data margin-right-10"
        ...
        >
          <ul>
            <li
              v-for="item in totalStatisticsList"
              :class="item.class"
            ...
            >
              <span class="home-head-data-label" style="font-weight:bold;text-shadow:0px 0px 10px #ffffff;">{{item.label}}</span>
              <span class="home-head-data-content" style="color:#ffbf49;text-shadow:0px 0px 20px #ffbf49;">{{item.value}}</span>
            </li>
          </ul>
        </div>
        ...
        <div
          class="home-head-chart home-head-data margin-left-10"
          ref="chart"
          v-loading="loading2"
          element-loading-text="数据加载中..."
          element-loading-background="rgba(0, 0, 0, 0.8)"
          style="border-radius:10px;"
        >
        <div style="color:white;font-size:14px;text-align:center;font-weight:bold">最近15天交易量</div>
          <v-chart
            ref="linechart"
            :type="'line'"
            :id="'homeId'"
      ...
      <!--Node statistics-->
      <!-- <div class="home-center"  style="border:1px solid red">
        <el-table
          :data="TbNodesList"
          :header-cell-style="bgTable"
        ...
        >
          <el-table-column label="节点Id" :show-overflow-tooltip="true" prop="nodeId" align="center">
            <template slot-scope="scope">
              <i
                class="wbs-icon-baocun copy-public-key"
                style="font-size: 15px;cursor:pointer"
                @click="copyPubilcKey(scope.row.nodeId)"
                title="复制"
              ></i>
      ...
      <div class="home-foot"  >
        <!--Block list-->
        <div class="home-foot-box margin-right-10"  style="border-radius:10px">
          ...
          >
            <ul>
              <li class="item" v-for="item in blockList" :key="item.number">
                <div class="left">
                  <div
                    @click="linkPage('blockDetail','blockHash',item.blockHash)"
                    class="table-link"
                  >区块 {{item.number}}</div>
                ...
                <div class="right">
                  ...
                  <div
                    class="txn"
                    @click="linkPage('transaction','blockHeight',item.number)"
                  >{{item.txn}} txns</div>
                </div>
              </li>
            </ul>
        ...
        <!--transaction list-->
        <div class="home-foot-box margin-left-10"  style="border-radius:10px">
          <div class="home-foot-box-nav">
            <div class="left">
              <span class="line"></span>
            ...
            <ul>
              <li class="item" v-for="itemlist in transactionList" :key="itemlist.blockTimesStr">
                <div class="left">
                  <div
                    class="transaction"
                    @click="linkPage( 'transactionDetail','pkHash',itemlist.transHash)"
                  >
                    交易
                    <span
                      class="table-link"
                      :title="itemlist.transHash | toUpperCase"
                    >{{itemlist.transHash | toUpperCase}}</span>
                  </div>
                  <div>
...
export default {
  ...
  methods: {
    ...
    searchTbBlcokChainInfo: function() {
      this.loading1 = true;
      getTbBlcokChainInfo(this.groupId)
        .then(res => {
    ...
    searchTbNodesInfo: function() {
      ...
      getTbNodeConnection(data, query)
        .then(res => {
          this.loading3 = false;
          if (res.data.code === 0) {
            if (res.data.data && res.data.data.length) {
    ...
    searchTbTransactionInfo: function() {
      ...
      getTbTransactionInfo(data, {})
        .then(res => {
          this.loading5 = false;
          if (res.data.code === 0) {
            if (res.data.data && res.data.data.length) {
              for (let i = 0; i < res.data.data.length; i++) {
                res.data.data[i].blockTimesStr = res.data.data[
                  i
                ].blockTimesStr.slice(0, 19);
                let item = {
