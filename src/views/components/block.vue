					...
					<div class="search-nav">
						<div class="hashInput">
							<el-input placeholder="请输入区块哈希或块高" v-model="searchKeyValue" class="input-with-select">
								<el-button slot="append" icon="el-icon-search" @click="search" :disabled="submitDisabled">
								</el-button>
							</el-input>
						</div>
					</div>
					<div class="search-table" style="margin:20px;">
						<div class="mysearch-table">
							<el-table :data="blockList" v-loading="loading" element-loading-text="数据加载中..."
							element-loading-background="rgba(0, 0, 0, 0.8)" style="border-radius:10px">
								<el-table-column prop="number" label="块高" align="center" :class-name="'table-link'"
								:show-overflow-tooltip="true">
									<template slot-scope="scope">
										<span @click="linkPage('blockDetail','blockHash',scope.row.blockHash)">
											{{scope.row.number}}
										</span>
									</template>
								</el-table-column>
								<el-table-column prop="dateTimeStr" label="生成时间" min-width="120px" align="center"
								:show-overflow-tooltip="true">
								</el-table-column>
								<el-table-column prop="txn" label="交易数量" align="center" :class-name="'table-link'"
								:show-overflow-tooltip="true">
									<template slot-scope="scope">
										<span @click="linkPage('transaction','blockHeight',scope.row.number)">
											{{scope.row.txn}}
										</span>
									</template>
								</el-table-column>
								<el-table-column prop="sealer" label="出块者" min-width="100px" :show-overflow-tooltip="true"
								align="center">
								</el-table-column>
								<el-table-column prop="pkHash" label="哈希" min-width="350px" :show-overflow-tooltip="true"
								align="center" :class-name="'table-link'">
									<template slot-scope="scope">
										<span @click="linkPage('blockDetail','blockHash',scope.row.blockHash)">
											{{scope.row.blockHash}}
										</span>
						...
						<div class="search-pagation">
							<div style="line-height: 40px;margin:10px">
								<span>
									查询结果 :
								</span>
								<span>
									共计{{pagination.total}}条数据
								</span>
							</div>
							<el-pagination style="display: inline-block" layout="sizes,prev, pager, next"
  ...
  methods: {
    ...
    search: function() {
      ...
      if (this.searchKeyValue.length > 60) {
        ...
        reg.test(this.searchKeyValue) &&
        this.searchKeyValue.substring(0, 2) != "0x"
      ) {
        this.hashData = "";
            ...
            if (res.data.data && res.data.data.length) {
              for (let i = 0; i < res.data.data.length; i++) {
                if (res.data.data[i].dateTimeStr) {
                  res.data.data[i].dateTimeStr = res.data.data[
                    i
